...
#' @param pred.alg prediction algorithm, "Distribution", "Trajectory" or "simpleTrajectory"
...
#' pred_diff <- predict(est_diff, t = seq(0, 1, by = 0.1), b.fun.mat = function(phi, t, y) phi[,1])  # much faster
#' pred_diff2 <- predict(est_diff, which.series = "current", b.fun.mat = function(phi, t, y) phi[,1])
#' pred_diff3 <- predict(est_diff, which.series = "current", y.start = data[51], t = t[seq(51, 100, by = 5)], b.fun.mat = function(phi, t, y) phi[,1])
#' }
#' pred_diff <- predict(est_diff, Euler.interval = TRUE, b.fun.mat = function(phi, t, y) phi[,1])  # one step Euler approximation
...
setMethod(f = "predict", signature = "est.Diffusion",
          definition = function(object,
                                ...
                                cand.length = 1000, pred.alg = c("Distribution", "Trajectory", "simpleTrajectory"),
  ...
  if(pred.alg == "simpleTrajectory"){
    ...
    for(i in 1:sample.length){
      result[i,] <- drawData(t, cl = cl, start = y.start, plot.series = FALSE)
    }
    result <- result[,-1]
  }else{
    if(Euler.interval){
...
setMethod(f = "predict", signature = "est.hiddenDiffusion",
    ...
    if(pred.alg == "simpleTrajectory"){
      ...
      cl <- set.to.class("hiddenDiffusion", parameter = list(phi = phi.est, gamma2 = gamma2.est, sigma2 = sigma2.est), b.fun = b.fun, sT.fun = sT.fun)
      Ypred <- matrix(0, sample.length, n)
      for(i in 1:sample.length){
        he <- drawData(t, cl = cl, start = y.start[sample(K, 1)], plot.series = FALSE)
        result[i,] <- he$Z
        Ypred[i,] <- he$Y
      }
...
setMethod(f = "predict", signature = "est.hiddenmixedDiffusion",
    ...
    if(pred.alg == "simpleTrajectory"){
      ...
      if(which.series == "current"){
        ...
        cl <- set.to.class("Diffusion", parameter = list(phi = phi.est, gamma2 = gamma2.est), b.fun = b.fun, sT.fun = sT.fun)
        for(i in 1:sample.length){
          result[i,] <- drawData(t, cl = cl, start = y.start[sample(K, 1)], plot.series = FALSE)
        }
      }
      if(which.series == "new"){
        mu.est <- apply(as.matrix(object@mu[ind,]), 2, mean)
        ...
        for(i in 1:sample.length){
          phi.pred[i, ] <- rnorm(length(mu.est), mu.est, sqrt(Omega.est))
          cl <- set.to.class("Diffusion", parameter = list(phi = phi.pred[i, ], gamma2 = gamma2.est), b.fun = b.fun, sT.fun = sT.fun)
          result[i,] <- drawData(t, cl = cl, start = object@model$y0.fun(phi.pred[i,], t[1]), plot.series = FALSE)
        }
      }
      Ypred <- result
...
#' pred_NHPP <- predict(est_NHPP, variable = "PoissonProcess", pred.alg = "simpleTrajectory", M2pred = length(data$Times))
#'
#' @export
setMethod(f = "predict", signature = "est.NHPP",
          definition = function(object, variable = c("eventTimes", "PoissonProcess"),
                                ...
                                cand.length = 1000, pred.alg = c("Trajectory", "Distribution", "simpleTrajectory"),
    ...
    if( variable == "eventTimes"){
      ...
      if(pred.alg == "simpleTrajectory"){
        ...
        if(which.series == "new"){
            ...
            for(i in 1:sample.length){
              result[i, ] <- drawData(t, cl, plot.series = FALSE)$N
            }
...
## jump diffusion:
      ...
      dt <- diff(t)
      samples <- list(phi = as.matrix(object@phi[ind]), theta = object@theta[ind], gamma2 = object@gamma2[ind])
      if(missing(sample.length) | pred.alg == "Distribution") sample.length <- K

      if(pred.alg == "Distribution") method <- "vector"
      if(pred.alg == "Trajectory") method <- "free"

      VFun <- function(yn, yn_1, samples)  mean(pnorm(yn, yn_1+dt[1]*b.fun(samples$phi, t[1], yn_1) + h.fun(samples$theta)*dN[,1], sqrt(s.fun(samples$gamma2, t[1], yn_1)^2*dt[1])))
      ...
      for(i in 2:(n-1)){
        ...
        cand.Area <- range(result[,i-1]) + dt[i]*b.fun(apply(samples$phi, 2, mean), t[i], mean(result[,i-1])) +
...
# sampling of jump diffusion through drawData ...
    if(pred.alg == "simpleTrajectory"){
      if(missing(sample.length)) sample.length <- 100
      ...
      if(which.series == "new"){
        ...
        for(i in 1:sample.length){
          help <- drawData(t, cl, start = object@Y[1], plot.series = FALSE)
          result[i, ] <- help$Y[-1]
          Npred[i, ] <- help$N
        }
...
pred.base <- function(samples, VFun, dens, len = 100, x0, method = c("vector", "free"),
  ...
  if(method == "vector" & pred.alg == "Trajectory") print("does not make much sense")
