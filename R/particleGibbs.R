...
# tau - time points in which to predict, samples - list(phi, gamma2, sigma2), last - vector of last points of X (results of conditional SMC)
# dXt=b(phi,t,Xt)dt+gamma*var(t,Xt)dWt, cand.X and cand.y vectors of candidate areas
predictiveDist <- function(tau, samples, last, drift, var, cand.X, cand.y){
  if(missing(cand.X)) cand.X <- seq(-2, 2, length = 200)
  ...
  Xtau <- matrix(0,L,K)
  vec <- last
  prob <- matrix(0,K,length(cand.X))

  for(l in 1:L){
    cand_l <- cand.X + mean(vec)

    for(a in 1:K){
      prob[a,] <- dnorm(cand_l, vec[a]+drift(samples$phi[a,],tau[l],vec[a])*Dt[l], sqrt(samples$gamma2[a]*Dt[l])*var(tau[l],vec[a]))
    }
  ...
  if(missing(cand.y)){
    he <- range(Xtau) + c(-1, 1)*sqrt(mean(samples$sigma2))*5
    cand.y <- seq(he[1], he[2], length = 200)
  }
  ...
  pred <- function(a){
    ...
    for(i in 1:K){
      mat[,i] <- dnorm(cand, Xtau[a,i], sqrt(samples$sigma2[i]))
    }
